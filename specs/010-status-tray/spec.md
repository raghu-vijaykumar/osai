...
### User Story 5 - Quick Open OSAI Home (Priority: P3)

The tray menu provides quick links to open the OSAI Home screen, open history, access settings, and quit OSAI entirely.

**Why this priority**: The tray becomes the launch point for all OSAI interactions. One-click access to Home reduces friction.

**Independent Test**: Right-click tray icon, click "Open Home", and verify the OSAI desktop application opens.

**Acceptance Scenarios**:

1. **Given** the tray app is running, **When** the user selects "Open Home", **Then** the OSAI desktop app launches (or gets focus if already open)
...
- **FR-003**: App MUST provide a right-click context menu with: Toggle Capture (pause/resume), Capture Settings (opens centralized capture controls from spec 063), Open Home, Open History, Settings, Quit
...
- **FR-012**: App MUST communicate with the OSAI daemon via the named pipe (Windows) or Unix socket — see protocol §7
...
- **FR-016**: App MUST support a global Privacy Mode that pauses ALL capture (browser, IDE, file watcher, activity monitor) with a single action — implemented via the same `control` IPC mechanism defined in spec 063, sending `pause` to every registered connector
- **FR-017**: Privacy Mode MUST be toggleable via: tray menu ("Privacy Mode"), keyboard shortcut (configurable, default: Ctrl+Shift+P / Cmd+Shift+P), and Chat Bar (`> privacy mode on`)
...
- **FR-024**: A Privacy Mode history log (local only, never synced) MUST track: `enabled_at`, `duration`, `source` (shortcut/tray/chat bar), and `auto_resumed` (boolean)
...
- **TrayMenu**: The right-click context menu with: Toggle Capture, Connectors, separator, Open Home, Open History, separator, Settings, Quit.
...
- Communicates with OSAI daemon via named pipe (Windows: `\\.\pipe\osai`) or Unix socket (`/tmp/osai.sock`) — see protocol §7
- Per-connector toggle and schedule controls are handled by the centralized Capture Settings page (spec 063) — the tray shows global controls only
